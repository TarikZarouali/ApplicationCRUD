<?php
    class SollicitatieModel
    {
        ...
        /**
         * Fetch the all Sollicitaties from database by using SqlServer stored procedure..
         * @return mixed
         */
        public function getSollicitatiesUseSpSqlServer() : array
        {
            try
            {
                // Use sql script to fetch all Sollicitaties from Sollicitatie database.
                $getAllSollicitatiesQuery = "EXEC [dbo].[spGetSollicitaties]";
        ...
        /**
         * Fetch the selected Sollicitatie from database by using SqlServer stored procedure..
         * @param [type] $id
         * @return SollicitatieEntityViewModel
         */
        public function getSollicitatieByIdUseSPSqlServer(int $id) : SollicitatieEntityViewModel
        {
            try
            {

                // Call sp from database Sollicitatie.
                $getSelectedApplicant = "EXEC [dbo].[spGetSollicitatieById] @id  = :id";
                $this->Db->query($getSelectedApplicant);
                $this->Db->bind(':id', $id);
                $result = $this->Db->single();
        ...
        /**
         * Modify the selected Sollicitatie in database by using SqlServer stored procedure..
         *
         * @param SollicitatieEntityViewModel $selectedSollicitatie
         * @return boolean
         */
        public function updateSollicitatieUseSPSqlServer(SollicitatieEntityViewModel $selectedSollicitatie) : bool
        {  
            try 
            {
                // Call sp from database Sollicitatie.
                $spQuery = "EXEC [dbo].[spUpdateSollicitatie]    @id				= :id	
                                                                ,@voornaam			= :voornaam			
                                                                ,@achternaam		= :achternaam	
                                                                ,@sollicitantnummer	= :sollicitantnummer	
                                                                ,@bedrijfnaam		= :bedrijfnaam		
                                                                ,@bedrijfcode		= :bedrijfcode		
                                                                ,@straat			= :straat			
                                                                ,@huisnummer		= :huisnummer		
                                                                ,@toevoeging		= :toevoeging		
                                                                ,@postcode			= :postcode			
                                                                ,@plaats			= :plaats			
                                                                ,@gespreksdatum		= :gespreksdatum		
                                                                ,@gesprekstijd		= :gesprekstijd		
                                                                ,@isActief			= :isActief			
                                                                ,@opmerking			= :opmerking";			
        ...
        /**
         * Create new Sollicitatie in database by using SqlServer stored procedure..
         * @param SollicitatieEntityViewModel $newSollicitatie
         * @return boolean
         */
        public function createSollicitatieUseSPSqlServer(SollicitatieEntityViewModel $newSollicitatie) : bool
        {
            try
            {
                // Create new Sollicitatie in database.
                $spQuery = "EXEC [dbo].[spCreateSollicitatie]   @voornaam			= :voornaam			
                                                               ,@achternaam		    = :achternaam	
                                                               ,@sollicitantnummer	= :sollicitantnummer	
                                                               ,@bedrijfnaam		= :bedrijfnaam		
                                                               ,@bedrijfcode		= :bedrijfcode		
